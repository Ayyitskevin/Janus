...
migration. Broad database-family modes, a containing directory other than
`0700`, and other identity hazards are refused before open. Sticky protection is
not sufficient for the containing directory because SQLite may create a sidecar
whose name did not exist during inspection. A failed storage line is a
